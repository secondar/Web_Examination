...
public partial class Admin_Teacher_PL : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = new DataSet();
            ...
            for (int i = 1; i < GridView1.Rows.Count; i++)
            {
                try
                {
                    SqlDataReader read = db.ExceRead("select * from Teacher where ID='" + GridView1.Rows[i].Cells[0].Text.ToString() + "'");
                    read.Read();
                    if (read.HasRows)
                    {
                    ...
                    {
                        db.eccom("insert into Teacher(id,name,pwd,JoinTime,degree) values ('" + GridView1.Rows[i].Cells[0].Text.ToString() + "','" + GridView1.Rows[i].Cells[1].Text.ToString() + "','" + GridView1.Rows[i].Cells[2].Text.ToString() + "' ,'" + DateTime.Now.ToString() + "','" + GridView1.Rows[i].Cells[3].Text.ToString() + "')");
                        A++;//导入成功
