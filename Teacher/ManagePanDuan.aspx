          ...
          <asp:Repeater ID="Repeater_dh" runat="server" DataSourceID="SqlDataSource1">
            <ItemTemplate>
              <tr class="text-c">
                ...
                <td><%#Eval("c_id") %></td>
                <td><%#Eval("Title") %></td>
                <td><a title="编辑" href="ChangePanDuan.aspx?ID=<%#Eval("ID") %>" Class="btn btn-primary radius" style="text-decoration:none">编辑</a> <a title="删除" href="javascript:;" onclick="Delete(this,'<%#Eval("ID") %>')" class="btn btn-danger radius" style="text-decoration:none">删除</a></td>
              </tr>
            </ItemTemplate>
          </asp:Repeater>
...
<script type="text/javascript">
    function datadelAll() {
        ...
        layer.confirm('确认要删除选中项目吗？', function (index) {
            $.ajax({
                url: "ManagePanDuan.aspx/DeleteAll",
                type: "post",
                data: "{'s':'" + s2 + "'}",
                dataType: 'json',
                ...
                success: function (data) {
                    ...
                    setTimeout("location.href = 'ManagePanDuan.aspx'", 1000);
                },
                error: function (data) {
